...
//Global declaration? WHY? - Kartik
var suiteFrame;
var testFrame;

//classes with cyclic dependency
var  HtmlTestCase,HtmlTestSuiteRow,TestResult,Metrics,HtmlRunnerTestLoop;
...
objectExtend(HtmlTestSuite.prototype, {
    ...
    reset: function() {
        ...
        var i;
        for (i = 0; i < this.suiteRows.length; i++) {
    ...
    isAvailable: function() {
        ...
        return testTable !== null && testTable !== undefined;
    },

    _collectSuiteRows: function () {
        var result = [];
        var tables = sel$A(this.suiteDocument.getElementsByTagName("table"));
        var testTable = tables[0],tableNum,rowNum,rowElement,newRow;
        ...
        for (tableNum = 1; tableNum < sel$A(this.suiteDocument.getElementsByTagName("table")).length; tableNum++) {
            testTable = tables[tableNum];
            for (rowNum = 1; rowNum < testTable.rows.length; rowNum++) {
                rowElement = testTable.rows[rowNum];
                newRow = new HtmlTestSuiteRow(rowElement, testFrame, this);
            }
        }
        return result;
    },
...
objectExtend(HtmlTestRunner.prototype, {
    ...
    loadSuiteFrame: function() {
        ...
        if (selenium === null || selenium === undefined) {
            var loc, appWindow = this._getApplicationWindow();
            try { loc = appWindow.location; }
            catch (e) { 
...
objectExtend(HtmlTestCase.prototype, {
    ...
    _collectCommandRows: function () {
        ...
        var i,j,table,tableRows,candidateRow,self = this;
        for (i = 0; i < tables.length; i++) {
            table = tables[i];
            tableRows = sel$A(table.rows);
            for (j = 0; j < tableRows.length; j++) {
                candidateRow = tableRows[j];
    ...
    reset: function() {
        ...
        this.setStatus('');
        var i;
        for (i = 0; i < this.commandRows.length; i++) {
...
//
    ...
    post: function () {
        ...
        if (resultsUrlQueryString) {
            var keyValuePair,key,value,i,clauses = resultsUrlQueryString.split('&');
            for (i = 0; i < clauses.length; i++) {
                keyValuePair = clauses[i].split('=', 2);
    ...
    _saveToFile: function (fileName, form) {
        ...
        scriptFile.WriteLine("<body>\n<h1>Test suite results</h1>" +
             ...
             "<tr>\n<td>" + inputs.suite + "</td>\n<td>&nbsp;</td>\n</tr></table><table>");
        var rowNum,testNum = inputs.numTestTotal;
        
        for (rowNum = 1; rowNum <= testNum; rowNum++) {
...
HtmlRunnerTestLoop = classCreate();
...
objectExtend(HtmlRunnerTestLoop.prototype, {
    ...
    _checkExpectedFailure : function(result) {
        if (this.expectedFailure !== null && this.expectedFailure !== undefined) {
