...
Recorder.addEventHandler('clickLocator', 'click', function(event) {
		if (event.button == 0) {
			...
			if (clickable) {
				...
				{
					info = event.target.title;
				}
				
                if (this.mouseoverLocator) {
                    this.record('mouseOver', this.mouseoverLocator, info);
                    delete this.mouseoverLocator;
                }
                this.record("click", this.findLocators(event.target), info);
            } else {
                var target = event.target;
                this.callIfMeaningfulEvent(function() {
                        this.record("click", this.findLocators(target), info);
                    });
            }
		}
	}, { capture: true });
