		...
		if (self.openCalled) {
            ...
            for (var i = 0; i < self.observers.length; i++) {
                if (self.observers[i].isSidebar) {
                    self.observers[i].getUserLog().warn("Actions in popup window cannot be recorded with BrighTest-Recorder in sidebar. Please open BrighTest-Recorder as standalone window instead to record them.");
                }
            }
			return result;
		}
...
Recorder.register = function(observer, window) {
	...
	if (!recorder) {
        ...
        if (window.wrappedJSObject) {
            // adding recorder to wrappedJSObject to make it visible from functional test of BrighTest-Recorder itself
