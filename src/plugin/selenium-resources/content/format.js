...
function FormatCollection(options) {
	...
	this.presetFormats = [ new InternalFormat(options, "HTML", "HTML", "html.js")];
	this.reloadFormats();
}
...
Format.prototype.loadFile = function(file, isURL) {
    ...
    if (filePath.indexOf(".xls") > 0) {
		...
		var testInfo = FormatterAddons.getTest(filePath);
        var rawCommands = testInfo.commands;
		for (var i = 0; i < rawCommands.length; i++) {
			var commandString = rawCommands[i];
			var splits = commandString.split("~");
			var command = new Command(splits[0], splits[1], splits[2]);
			commands[i] = command;
		}
		testCase.commands = commands;
        var additionalDesc = testInfo.desc;
        testCase.setAdditionalParams(additionalDesc[0], additionalDesc[1], additionalDesc[2]);
    } else {
