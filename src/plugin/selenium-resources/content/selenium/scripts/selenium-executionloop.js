...
TestLoop.prototype = {
    ...
    _executeNextDSLCommand: function () {
        if (this.currentDSLCommand.isAtLastCommand()) {
            this.currentDSLCommand = null;
            this.continueTest();
        } else {
            this.currentCommand = this.currentDSLCommand.nextCommand();
            this._executeCurrentCommand();
            this._continueWhenDSLCommandIsComplete();
        }
    },
    // AN Hack alert can not move this to debugger-extension as we need selenium
    _continueWhenDSLCommandIsComplete: function () {
        selenium.browserbot.runScheduledPollers();
        if (this.waitForCondition == null) {
            ...
            window.setTimeout(fnBind(this._continueWhenDSLCommandIsComplete, this), 10);
        }
    },
