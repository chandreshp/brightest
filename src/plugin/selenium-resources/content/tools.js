...
function Log(category) {
	...
	this.log = function(level, msg) {
        ...
        //AN: hack as the var threshold was returned as null, this can happen if we have a preference other than one of the valid values for internalLogThreshold
        if (!threshold) {
            threshold = this.INFO;
        }
		if (level.level >= threshold.level) {
			this._write("BrighTest-Recorder [" + level.name + "] " + 
...
Log.prototype = {
    _getThreshold: function() {
        if (!this.threshold) {
        	...
        	//AN: for ease of understanding
            //this.threshold = Preferences.getString("internalLogThreshold", "DEBUG");
        }
    ...
    _write: function(message) {
    	//AN: window switching is a killer	
	    if (Components) {
