...
function Editor(window) {
    ...
    this.app.addObserver({
            ...
            if (Editor.GENERIC_AUTOCOMPLETE) {
...
Editor.DEFAULT_TITLE = "BrighTest IDE";
...
Editor.prototype.unload = function() {
    ...
    try{
    	...
    	//AN sacrificed for migration to 4.0x
    }
...
Editor.prototype.getPathAndUpdateBaseURL = function(window) {
	...
	if (result && "true" != this.getOptions().recordAbsoluteURL && "true" == this.getOptions().recordAbsoluteURL ) {
...
Editor.prototype._getTopWindow = function(window) {
    if (this.topWindow) {
        var top = this.topWindow; // for functional test of BrighTest-Recorder
 ...
 * Used for self-testing BrighTest-Recorder.
 */
...
Editor.prototype.loadExtensions = function() {
	...
	if (this.getOptions().ideExtensionsPaths) {
		try {
			...
			this.showAlert("error loading BrighTest-Recorder extensions: " + error);
		}
	}
}
...
//Editor.references.push(new HTMLReference("Japanese", "Reference HTML contained in BrighTest-Recorder", "http://wiki.openqa.org/display/SEL/Selenium+0.7+Reference+%28Japanese%29"));
